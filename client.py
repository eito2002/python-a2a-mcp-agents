...
from python_a2a.models import Message, Conversation, MessageRole, TextContent, ErrorContent
...
class A2ANetworkClient:
    ...
    def _create_error_message(self, error_text: str, original_message: Optional[Message] = None) -> Message:
        """
    ...
    def send_message(self, message: Message, agent_name: Optional[str] = None) -> Message:
        """
    ...
    def run_workflow(self, initial_message: Message, workflow: List[str]) -> Conversation:
        """
        ...
        for agent_name in workflow:
            if agent_name not in self.agents:
                conversation.create_error_message(f"Agent '{agent_name}' not found in the network")
                return conversation
        ...
        for agent_name in workflow:
            ...
            try:
                ...
                conversation.create_error_message(f"Error communicating with {agent_name} agent: {str(e)}")
                break
