...
TRAVEL_DATA = {
    "london": {
        ...
        "indoor_activities": ["British Museum", "National Gallery", "Madame Tussauds", "Shopping at Harrods", "West End Theatre"],
        "outdoor_activities": ["Hyde Park", "Thames River Cruise", "Tower Bridge", "Camden Market", "Kew Gardens"],
        "cuisines": ["British", "Indian", "French", "Italian", "Chinese"],
        "transportation": ["Underground (Tube)", "Bus", "Taxi", "Rental Bikes", "Walking"],
        "safety": "High",
        "language": "English",
    ...
    "paris": {
        ...
        "indoor_activities": ["Louvre Museum", "Musée d'Orsay", "Centre Pompidou", "Galeries Lafayette", "Parisian Cafés"],
        "outdoor_activities": ["Seine River Cruise", "Luxembourg Gardens", "Champs-Élysées", "Montmartre Walk", "Tuileries Garden"],
    ...
    "new york": {
        ...
        "indoor_activities": ["Metropolitan Museum of Art", "MoMA", "American Museum of Natural History", "Broadway Shows", "Shopping in SoHo"],
        "outdoor_activities": ["Central Park", "High Line", "Brooklyn Bridge Walk", "Staten Island Ferry", "Coney Island"],
    ...
    "tokyo": {
        "attractions": ["Tokyo Tower", "Senso-ji Temple", "Meiji Shrine", "Shibuya Crossing", "Tokyo Skytree"],
        "indoor_activities": ["Tokyo National Museum", "TeamLab Borderless", "Shopping in Ginza", "Robot Restaurant", "Japanese Food Markets"],
        "outdoor_activities": ["Yoyogi Park", "Shinjuku Gyoen", "Tsukiji Fish Market", "Sumida River Cruise", "Ueno Park"],
    ...
    "sydney": {
        "attractions": ["Sydney Opera House", "Sydney Harbour Bridge", "Bondi Beach", "Taronga Zoo", "Darling Harbour"],
        "indoor_activities": ["Art Gallery of NSW", "Australian Museum", "Queen Victoria Building", "Sydney Tower Eye", "Sea Life Sydney Aquarium"],
        "outdoor_activities": ["Bondi to Coogee Coastal Walk", "Sydney Harbour Cruise", "Royal Botanic Garden", "Blue Mountains Day Trip", "Manly Beach"],
        "cuisines": ["Australian", "Asian Fusion", "Mediterranean", "Seafood", "Italian"],
...
def suggest_activities(location: str, weather_condition: str):
    ...
    bad_weather_keywords = ["rain", "snow", "storm", "thunder", "cold", "windy", "hurricane", "tornado", "typhoon"]
    is_bad_weather = any(keyword in weather_condition.lower() for keyword in bad_weather_keywords)
...
def create_trip_itinerary(location: str, days: int = 3, weather_condition: str = None):
    ...
    for day in range(1, days + 1):
        ...
        if is_bad_weather or (day % 2 == 0):  # Alternate or weather-based
            if indoor:
                ...
                daily_plan["morning"] = {"activity": attractions.pop(0), "type": "attraction"}
        else:
            if outdoor:
        ...
        if is_bad_weather:
            if indoor:
                ...
                daily_plan["afternoon"] = {"activity": attractions.pop(0), "type": "attraction"}
        else:
            if day % 2 == 0 and outdoor:
                daily_plan["afternoon"] = {"activity": outdoor.pop(0), "type": "outdoor"}
            elif indoor:
                daily_plan["afternoon"] = {"activity": indoor.pop(0), "type": "indoor"}
            elif attractions:
                daily_plan["afternoon"] = {"activity": attractions.pop(0), "type": "attraction"}
        ...
        daily_plan["evening"] = {"activity": f"Dinner - {random.choice(data['cuisines'])} cuisine", "type": "dining"}
        
        # Meals
        daily_plan["meals"] = [
            {"meal": "Breakfast", "suggestion": "Hotel or local café"},
            {"meal": "Lunch", "suggestion": f"Try {random.choice(data['cuisines'])} food"},
            {"meal": "Dinner", "suggestion": f"{random.choice(data['cuisines'])} restaurant"}
        ]
        
        # Add transportation tip
        daily_plan["transportation_tip"] = f"Best way to get around: {random.choice(data['transportation'])}"
    ...
    result = {
        "location": location.title(),
        "trip_duration": days,
        "weather_consideration": weather_condition if weather_condition else "Not specified",
        "itinerary": itinerary,
        "tips": [
