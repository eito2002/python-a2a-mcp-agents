...
import json
from config import logger, OPENAI_AVAILABLE
...
class AIRouter:
    ...
    def __init__(self, agent_network, model=None):
        ...
        if not OPENAI_AVAILABLE:
            logger.warning("OpenAI integration not available. AI routing will not work.")
            return
                ...
                if agent_card:
                    ...
                    if hasattr(agent_card, 'skills'):
                        for skill in agent_card.skills:
                            skill_info = {
                                "name": skill.name if hasattr(skill, 'name') else "Unknown skill",
