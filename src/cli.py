...
MCP_SERVER_CONFIGS = [
    ...
    {"module": "mcp_servers.maps_mcp_server", "server_var": "maps_mcp", "port": 5002},
    {
...
async def start_single_agent(args):
    ...
    if args.connect_to:
        for conn_spec in args.connect_to:
            if ":" not in conn_spec:
                logger.error(
                    f"Invalid connection specification: {conn_spec}. Format should be agent_name:port"
                )
                return
            agent_name, agent_port_str = conn_spec.split(":", 1)
            try:
                agent_connections[agent_name] = (
                    f"http://localhost:{int(agent_port_str)}"
                )
                logger.info(
                    f"Adding connection to {agent_name} at port {agent_port_str}"
                )
            except ValueError:
    ...
    try:
        ...
        if merged_connections:
            agent = agent_class(
                mcp_servers=mcp_servers_config, agent_connections=merged_connections
            )
        else:
        ...
        # Register agent info
        extra_info = {"is_mcp": True, "mcp_servers": list(mcp_servers_config.keys())}
        ...
        AgentServer.register_agent(
            name=args.agent, agent=agent, port=agent_port, **extra_info
        )

        print(
            f"{args.agent} agent is running on port {agent_port}. Press Ctrl+C to stop."
        )
        print(
            f"Use 'python -m cli query \"Your query here\" --agent {args.agent} --agent-ports {args.agent}:{agent_port}' to query this agent."
        )
...
def query_agent(args):
    ...
    if args.agent_ports:
        for port_spec in args.agent_ports:
            if ":" not in port_spec:
                print(
                    f"Error: Invalid port specification: {port_spec}. Format should be agent_name:port"
                )
                return
    ...
    if not client.agents:
        print(
            "Error: No agents available. Use 'start-agent' command first, or specify agent ports."
        )
        return
    ...
    message = Message(content=TextContent(text=args.query), role=MessageRole.USER)
...
def main():
    ...
    # Start-agent command
    start_agent_parser = subparsers.add_parser(
        "start-agent", help="Start a single agent with configurable connections"
    )
    start_agent_parser.add_argument(
        "--agent", required=True, help="Agent to start (mcp_weather, mcp_travel)"
    )
    start_agent_parser.add_argument(
        "--port",
        type=int,
    ...
    query_parser.add_argument(
        "--agent", help="Specific agent to query (mcp_weather, mcp_travel)"
    )
    query_parser.add_argument(
        "--router-type",
        default="keyword",
