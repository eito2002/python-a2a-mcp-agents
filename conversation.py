...
class ConversationOrchestrator:
    ...
    def _process_next_step(self, conversation_id: str, current_input: str) -> None:
        ...
        conversation["messages"].append(
            {
                "role": (
                    "user"
                    if current_step == 0
                    else conversation["workflow"][current_step - 1]
                ),
        ...
        try:
            ...
            logger.info(
                f"Sending message to {current_agent} in conversation {conversation_id}"
            )
            ...
            conversation["messages"].append(
                {"role": current_agent, "content": response_text}
            )
            ...
            logger.error(
                f"Error in conversation {conversation_id} with agent {current_agent}: {e}"
            )
            # Mark the conversation as complete with an error
            conversation["complete"] = True
            conversation["result"] = (
                f"Error in conversation with agent {current_agent}: {str(e)}"
            )
